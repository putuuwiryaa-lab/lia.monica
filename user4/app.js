...
// Set insertion order records selection clicks; reselecting moves a market last.
const selectedMarkets = new Set();
function setMarketSelected(market, selected) {
  market.selected = selected;
  if (selected) selectedMarkets.add(market);
...
async function prosesPasaran(){const boxes=[...selectedMarkets],em=document.getElementById('error-msg'),out=document.getElementById('output-container'),btn=document.getElementById('process-button');if(!boxes.length){em.textContent='Silakan pilih minimal satu pasaran.';em.style.display='block';return}em.style.display='none';btn.disabled=true;btn.textContent='MENGHITUNG...';btn.style.opacity='.7';out.innerHTML='<div class="loading" style="padding:30px 10px;text-align:center"><span style="display:inline-block;width:18px;height:18px;border:2px solid var(--line);border-top-color:var(--accent);border-radius:50%;animation:spin .7s linear infinite;vertical-align:middle;margin-right:8px"></span>Menghitung hasil...</div>';requestAnimationFrame(()=>out.scrollIntoView({behavior:'smooth',block:'start'}));try{const data=await Promise.all(boxes.map(m=>api(`/api/markets/${encodeURIComponent(m.id)}`)));allResults=[];const lines=[];for(let i=0;i<boxes.length;i++){const m=data[i],name=String(m.name||boxes[i].name||m.id).toUpperCase(),h=history(m.history_data,m.last_result);if(h.length<3){lines.push(`${esc(name)} ⟢ DATA BELUM CUKUP (MINIMAL 3 RESULT)`);continue}const r=runAPM4D(h),value=r.digits.join('');allResults.push(`${name} ⟢ ${value}`);lines.push(`${esc(name)} <span class="result-sep">⟢</span> <span class="result-digits">${value}</span>`)}const text=allResults.join('\n');out.innerHTML=`<div class="market-result-card compact-result"><pre>${lines.join('\n')}</pre></div><div class="button-group"><button class="btn-copy" id="copy-ai">Salin Hasil BBFS 7 Digit</button></div>`;document.getElementById('copy-ai')?.addEventListener('click',()=>copyText(text,'Hasil BBFS 7 Digit Disalin!'));requestAnimationFrame(()=>out.scrollIntoView({behavior:'smooth',block:'start'}))}catch(e){out.innerHTML=`<div style="text-align:center;color:#f87171;padding:15px;font-size:.78rem">Gagal mengambil data: ${esc(e.message)}</div>`;out.scrollIntoView({behavior:'smooth',block:'start'})}finally{btn.disabled=false;btn.textContent='PROSES HASIL';btn.style.opacity='1'}}
document.getElementById('process-button')?.addEventListener('click',prosesPasaran);document.getElementById('select-all')?.addEventListener('click',()=>{marketData.forEach(m=>setMarketSelected(m,true));renderMarkets();updateCount()});document.getElementById('clear-all')?.addEventListener('click',()=>{marketData.forEach(m=>setMarketSelected(m,false));renderMarkets();updateCount()});document.getElementById('market-search')?.addEventListener('input',renderMarkets);document.getElementById('logout-button')?.addEventListener('click',logout);const s=document.createElement('style');s.textContent='.compact-result{padding:12px 13px}.compact-result pre{font-size:.88rem;line-height:1.75;letter-spacing:.3px}.result-sep{color:#64748b;padding:0 3px}.result-digits{color:#4ade80;font-weight:800} @keyframes spin{to{transform:rotate(360deg)}}';document.head.appendChild(s);loadMarkets();