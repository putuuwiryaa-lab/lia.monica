...
function pageHeaders(extra: Record<string, string> = {}) {
...
function clientIp(req: Request) {
  return (req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "unknown").slice(0, 100);
}
...
function limited(
  ...
  if (!current || now >= current.reset) {
    store.set(key, { count: 1, reset: now + RATE_WINDOW_MS });
    return false;
  }
...
function rateLimited(req: Request) {
  return limited(requests, clientIp(req), RATE_LIMIT);
}

function loginRateLimited(req: Request) {
  return limited(loginRequests, clientIp(req), LOGIN_RATE_LIMIT);
}
...
async function sign(value: string) {
  if (!SESSION_SECRET) {
    throw new Error("LOGIN_SESSION_SECRET belum dikonfigurasi.");
  }
...
async function makeSession(user: string) {
...
async function getSession(req: Request) {
  ...
  try {
    ...
    if (!data?.u || Date.now() > Number(data.e)) return null;
...
function sessionCookie(token: string) {
  return `lia_session=${token}; Path=/; Max-Age=${SESSION_MAX_AGE}; HttpOnly; Secure; SameSite=Strict`;
}

function clearCookie() {
  return "lia_session=; Path=/; Max-Age=0; HttpOnly; Secure; SameSite=Strict";
}

async function servePage(req: Request, path: string) {
  return serveDir(
    new Request(new URL(path, req.url), req),
    { fsRoot: ".", showIndex: true },
  );
}
...
Deno.serve(async (req) => {
  ...
  if (url.pathname === "/api/login" && req.method === "POST") {
    ...
    try {
      ...
      if (!['user1', 'user2'].includes(user) || password.length === 0 || password.length > 256) {
        return json({ ok: false, error: "Login tidak valid." }, 401);
      }

      const expectedUserPassword = String(
        user === "user1"
          ? Deno.env.get("LOGIN_PASS1") ?? ""
          : Deno.env.get("LOGIN_PASS2") ?? "",
      );

      const validMaster = MASTER_PIN.length > 0 && password === MASTER_PIN;
      const validUser = expectedUserPassword.length > 0 && password === expectedUserPassword;

      if (!validMaster && !validUser) {
      ...
      return json(
        {
          ok: true,
          user,
          redirect: user === "user1" ? "/user1/" : "/user2/",
        },
        200,
  ...
  if (url.pathname === "/api/health" && req.method === "GET") {
    return json({
      ...
      master_pin_configured: Boolean(MASTER_PIN),
    });
  }
  ...
  if (url.pathname === "/api/markets" && req.method === "GET") {
    ...
    try {
      return json(
        await supabaseGet("markets?select=id,name,order,last_result&order=order.asc"),
      );
    } catch (error) {
  ...
  if (url.pathname.startsWith("/api/markets/") && req.method === "GET") {
    ...
    try {
      const markets = await supabaseGet(
        `markets?select=id,name,history_data,last_result&id=eq.${encodeURIComponent(id)}&limit=1`,
      );
  ...
  if (url.pathname === "/config.js") {
    return new Response(
      "window.SUPABASE_URL=undefined;window.SUPABASE_ANON_KEY=undefined;",
      {
        status: 200,
        headers: {
  ...
  if (url.pathname === "/" || url.pathname === "/index.html") {
    if (!user) {
      return new Response(null, {
        status: 302,
        headers: { location: "/login.html", ...pageHeaders() },
      });
    }
    ...
    if (user !== "user1") {
      return new Response(null, {
        status: 302,
        headers: { location: "/user2/", ...pageHeaders() },
      });
    }
  ...
  if (url.pathname === "/user1" || url.pathname === "/user1/") {
    if (user !== "user1") {
      return new Response(null, {
        status: 302,
        headers: {
          location: user === "user2" ? "/user2/" : "/login.html",
          ...pageHeaders(),
        },
      });
    }

    const response = await servePage(req, "/index.html");
  ...
  if (url.pathname === "/user2" || url.pathname === "/user2/") {
    if (user !== "user2") {
      return new Response(null, {
        status: 302,
        headers: {
          location: user === "user1" ? "/user1/" : "/login.html",
          ...pageHeaders(),
        },
      });
    }

    const response = await servePage(req, "/user2/index.html");
    return new Response(response.body, {
      status: response.status,
      headers: response.headers,
    });
